DIST := dist
IMPORT := code.github.io/khmarbaise/filegenerator
...
ifneq ($(DRONE_TAG),)
	...
	FGEN_VERSION ?= $(shell git describe --tags --always | sed 's/-/+/' | sed 's/^v//')
endif

LDFLAGS := -X "main.Version=$(FGEN_VERSION)" -X "main.Tags=$(TAGS)"

GO_DIRS := modules vendor
...
release-os:
	...
	CGO_ENABLED=0 gox -verbose -cgo=false -tags '$(TAGS)' -ldflags '-s -w $(LDFLAGS)' -osarch='!darwin/386 !darwin/arm64 !darwin/arm' -os="windows linux darwin" -arch="386 amd64 arm arm64" -output="$(DIST)/release/linkelist-$(VERSION)-{{.OS}}-{{.Arch}}"
